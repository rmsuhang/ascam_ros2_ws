...
int CameraPublisher::start()
{
    ...
    if (server == nullptr) {
        server = new CameraSrv(this, config_path);// 创建相机服务实例
        ret = server->start();
        if (ret != 0) {
            RCLCPP_ERROR_STREAM(m_logger, "start server failed");
        }
        m_monitor_flg = true;
        m_monitor_thd = std::thread(&CameraPublisher::streamController, this);//创建新线程，用于监控和控制数据流
    }
...
void CameraPublisher::onCameraNewFrame(AS_CAM_PTR pCamera, const AS_SDK_Data_s *pstData)
{
    ...
    if (ret == 0) {
        depthInfoPublisher(pCamera, pstData, stIrRgbParameter, time);// 发布深度图像
        pointcloudPublisher(pCamera, pstData, stIrRgbParameter);// 发布点云数据
        rgbInfoPublisher(pCamera, pstData, stIrRgbParameter, time);// 发布RGB图像
        irInfoPublisher(pCamera, pstData, stIrRgbParameter, time);// 发布红外图像
        peakInfoPublisher(pCamera, pstData, stIrRgbParameter, time);// 发布峰值图像
        tfPublisher(pCamera, stIrRgbParameter);// 发布TF变换数据
    }
}
        ...
        if (it->pCamera == pCamera) {
            ...
            if (it_par != m_cam_type_map.end()) {
                if ((it_par->second == AS_SDK_CAM_MODEL_HP60C) || (it_par->second == AS_SDK_CAM_MODEL_HP60CN) ||
                    ...
                    geometry_msgs::msg::TransformStamped rgbTransform;//创建rgb到cam_link的tf
