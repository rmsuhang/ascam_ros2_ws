...
int CameraSrv::start()
{
    ...
    listener_callback.onAttached = onAttached;// 设置设备连接事件的回调函数
    listener_callback.onDetached = onAttached;  // 设置设备断开事件的回调函数（这里可能有个bug，应该是onDetached而不是onAttached）
    listener_callback.privateData = this; // 设置私有数据指针，指向当前CameraSrv实例

    AS_SDK_StartListener(listener_callback, AS_LISTENNER_TYPE_USB, true); // 启动USB设备监听器，一旦有设备连接或断开，就会进入回调函数
...
void CameraSrv::onAttached(AS_CAM_ATTR_S *attr, void *privateData)
{
    ...
    if (ret == 0) {
        ...
        server->m_camera_status->onCameraAttached(newdev, stream_param, cam_type);//根据相机型号创建对应的ROS2发布器，支持不同类型的数据发布（深度图、RGB图、点云等）
        if (stream_param.open) {
            AS_CAM_Stream_Cb_s streamCallback;
            streamCallback.callback = server->onNewFrame;// 注册新帧到达的回调函数
...
{
    CameraSrv *server = static_cast<CameraSrv *>(privateData);
    server->m_camera_status->onCameraNewFrame(pCamera, pstData);//处理新的帧数据
}
